...
class PancreasDataset:
    '''
    This class is for the pancreas segmentation dataset from the decathlon dataset.
    You can get more information about it using `info()` function.

    ### Example usage

    ```Python
    from pycad.dataset.segmentation.decathlon import PancreasDataset
    ...
    ```
    '''
    def __init__(self, dataset_size=281):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=1oAlyAG5A4E4docdVWheXIZjFeTijy1BR',
            '200': 'https://drive.google.com/uc?id=1n4yk70NN3yxWylTkblbXUIqzTSwQkzX2',
            'all': 'https://drive.google.com/uc?id=1t8RVaf8yoNBw5c55wUPG3HPc5b-nVHb-'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Pancreas Dataset from Decathlon dataset. This is a collection of CT scans with means these are 3D volumes.")
        print(f"Total Cases: {self.dataset_size}")
        print(f"Subgroups: 100, 200, {self.dataset_size}")

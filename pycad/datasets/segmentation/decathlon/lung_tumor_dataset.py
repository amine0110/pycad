...
class LungTumorDataset:
    '''
    This class is for the lung tumor segmentation dataset from the decathlon dataset.
    You can get more information about it using `info()` function.

    ### Example usage

    ```Python
    from pycad.dataset.segmentation.decathlon import LungTumorDataset
    
    lung_tumor_dataset = LungTumorDataset()
    lung_tumor_dataset.info()  # Print dataset information
    lung_tumor_dataset.download('all')  # Download and extract subgroup all (63 cases)
    ```
    '''
    def __init__(self, dataset_size=63):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            'all': 'https://drive.google.com/uc?id=1wzfBTfld1MgKHQnrbppEufPqqKz33w_R'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Lung Tumor Dataset from Decathlon dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'lung_tumor{subgroup}.zip')

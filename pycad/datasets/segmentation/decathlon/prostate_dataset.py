...
class ProstateDataset:
    '''
    This class is for the prostate segmentation dataset from the decathlon dataset.
    You can get more information about it using `info()` function.

    ### Example usage

    ```Python
    from pycad.dataset.segmentation.decathlon import ProstateDataset
    
    prostate_dataset = ProstateDataset()
    prostate_dataset.info()  # Print dataset information
    prostate_dataset.download('all')  # Download and extract subgroup all (32 cases)
    ```
    '''
    def __init__(self, dataset_size=32):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            'all': 'https://drive.google.com/uc?id=1rwjzWqc8RVGbxTVGmcnpzA2L4k-5C4lr'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Prostate Dataset from Decathlon dataset. This is a collection of CT scans with means these are 3D volumes.")

...
class LiverAndTumorDataset:
    '''
    This class is for the liver and tumor segmentation dataset from the decathlon dataset.
    You can get more information about it using `info()` function.

    ### Example usage

    ```Python
    from pycad.dataset.segmentation.decathlon import LiverAndTumorDataset
    
    liver_and_tumor_dataset = LiverAndTumorDataset()
    liver_and_tumor_dataset.info()  # Print dataset information
    liver_and_tumor_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=131):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=1z9HUe6VPv24ZHDQltfH0x8_oS8zbK4VR',
            'all': 'https://drive.google.com/uc?id=1GmS4JCYkF_2k12V0_MLMuvv_PkDkGrzq'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Liver And Tumor Dataset from Decathlon dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'liver_and_tumor{subgroup}.zip')

...
class VertebraeT5Dataset:
    '''
    This class is for the vertebrae t5 segmentation dataset from the total segmentator dataset.
    You can get more information about it using `info()` function.

    ### Example usage

    ```Python
    from pycad.dataset.segmentation.TotalSegmentator import VertebraeT5Dataset
    
    vertebrae_t5_dataset = VertebraeT5Dataset()
    vertebrae_t5_dataset.info()  # Print dataset information
    vertebrae_t5_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=1MRSOxyColahTTSNm0ru5P3FHdXFFf1ME',
            '200': 'https://drive.google.com/uc?id=1M5t4J1FrhddrpgrH38uIVo1lZIbXgjcu',
            '400': 'https://drive.google.com/uc?id=1urpMSWk1EVNiGw330q-92N920LL-85F_',
            'all': 'https://drive.google.com/uc?id=1gMeSS601e2Xogmjx-gKD8hIrSSoWbJAT'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Vertebrae T5 Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'vertebrae_t5{subgroup}.zip')

...
class HeartDataset:
    '''
    This class is for the heart segmentation dataset from the total segmentator dataset.
    ...
    heart_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=1LXxAwj9ooP19nOv6WUcUNG72elkBUL_X',
            '200': 'https://drive.google.com/uc?id=15liLcfbPNZYd0e0jEGWyyX-Lgyw8Xnln',
            '400': 'https://drive.google.com/uc?id=1aqRAdbo70gngaQwuVsK9qi3cYjodiKmz',
            'all': 'https://drive.google.com/uc?id=1htGQa00n4Z9gkuNKiBBf3jISBwvaTCF0'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Heart Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'heart{subgroup}.zip')

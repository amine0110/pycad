...
class RibRight11Dataset:
    '''
    This class is for the rib right 11 segmentation dataset from the total segmentator dataset.
    You can get more information about it using `info()` function.

    ### Example usage

    ```Python
    from pycad.dataset.segmentation.TotalSegmentator import RibRight11Dataset
    
    rib_right_11_dataset = RibRight11Dataset()
    rib_right_11_dataset.info()  # Print dataset information
    rib_right_11_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=1CUnik22rIdhLaiEOR6iF92iUpP031O4J',
            '200': 'https://drive.google.com/uc?id=1L_3h1CJSBMLX9Fg9TYZwRSgvJ63iP3tB',
            '400': 'https://drive.google.com/uc?id=1fIeXoVrPCZecoYBU1dr-h0FfsVOQDtl1',
            'all': 'https://drive.google.com/uc?id=1i-oQO6DYXpwIk8uhk1fgGXBkkOmEQ0ch'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Rib Right 11 Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'rib_right_11{subgroup}.zip')

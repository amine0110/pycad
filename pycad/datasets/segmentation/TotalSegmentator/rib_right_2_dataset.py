...
class RibRight2Dataset:
    '''
    This class is for the rib right 2 segmentation dataset from the total segmentator dataset.
    You can get more information about it using `info()` function.

    ### Example usage

    ```Python
    from pycad.dataset.segmentation.TotalSegmentator import RibRight2Dataset
    
    rib_right_2_dataset = RibRight2Dataset()
    rib_right_2_dataset.info()  # Print dataset information
    rib_right_2_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=1m35odJWNPD47TlMpj3uqjZCUEXwo5CBD',
            '200': 'https://drive.google.com/uc?id=1zrE4BJD3UA8Hd-iZcnu5GsxeNfpAg-lX',
            '400': 'https://drive.google.com/uc?id=1SrjeA5B_xtQOeR7xbPrwI05ZwJiPdkBn',
            'all': 'https://drive.google.com/uc?id=1auYGIoGiVdacnboLLTwcL0A9jvdi4G7J'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Rib Right 2 Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'rib_right_2{subgroup}.zip')

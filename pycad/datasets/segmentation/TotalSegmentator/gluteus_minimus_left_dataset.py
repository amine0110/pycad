...
class GluteusMinimusLeftDataset:
    '''
    This class is for the gluteus minimus left segmentation dataset from the total segmentator dataset.
    ...
    gluteus_minimus_left_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=1k6goEbh45BJepeFf98vb-HV8gpgUuNfO',
            '200': 'https://drive.google.com/uc?id=1af9u_9Sz2XoyydrxYcmKhJ7u8wlfRfcr',
            '400': 'https://drive.google.com/uc?id=1fDEvaRPDyKQqP70GbjpoLWMWhGCB9zEt',
            'all': 'https://drive.google.com/uc?id=1VcAydxERsr7zQwi0KrRG-VYutfa-jXTO'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Gluteus Minimus Left Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'gluteus_minimus_left{subgroup}.zip')

...
class UrinaryBladderDataset:
    '''
    This class is for the urinary bladder segmentation dataset from the total segmentator dataset.
    You can get more information about it using `info()` function.

    ### Example usage

    ```Python
    from pycad.dataset.segmentation.TotalSegmentator import UrinaryBladderDataset
    
    urinary_bladder_dataset = UrinaryBladderDataset()
    urinary_bladder_dataset.info()  # Print dataset information
    urinary_bladder_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=1JVq13cTBQu4PXPb5tvIbnfHXYDnTuszO',
            '200': 'https://drive.google.com/uc?id=1xshs-T2RWPeDxzEyYbAxMziCVMK2fHQA',
            '400': 'https://drive.google.com/uc?id=1AgiJk6B7qCgAOgtqIss_sCX71xeoztAG',
            'all': 'https://drive.google.com/uc?id=1oupQHa_PZ87WteA4oSet1McxF9SBPxhx'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Urinary Bladder Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'urinary_bladder{subgroup}.zip')

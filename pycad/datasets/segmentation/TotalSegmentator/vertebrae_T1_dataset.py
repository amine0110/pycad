...
class VertebraeT1Dataset:
    '''
    This class is for the vertebrae t1 segmentation dataset from the total segmentator dataset.
    You can get more information about it using `info()` function.

    ### Example usage

    ```Python
    from pycad.dataset.segmentation.TotalSegmentator import VertebraeT1Dataset
    
    vertebrae_t1_dataset = VertebraeT1Dataset()
    vertebrae_t1_dataset.info()  # Print dataset information
    vertebrae_t1_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=1Y-4mLzsf2Aro0lqvlIDGmhl15duiwPqT',
            '200': 'https://drive.google.com/uc?id=1vpa5rorOnYJWP_yjRJ9UGq__GdDnPB6F',
            '400': 'https://drive.google.com/uc?id=1dC51cxElGj48zM0HL9ZpTtAtSHIGvmsK',
            'all': 'https://drive.google.com/uc?id=1MxF1vAi8ecGgrkEtTbZL9pUsH54DZ13k'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Vertebrae T1 Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'vertebrae_t1{subgroup}.zip')

...
class RibLeft11Dataset:
    '''
    This class is for the rib_left_11 segmentation dataset from the total segmentator dataset.
    You can get more information about it using `info()` function.

    ### Example usage

    ```Python
    from pycad.dataset.segmentation.TotalSegmentator import RibLeft11Dataset
    
    rib_left_11_dataset = RibLeft11Dataset()
    rib_left_11_dataset.info()  # Print dataset information
    rib_left_11_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=1zlwl51GnDQM_-9j4L1BSEkDwdWppoJtx',
            '200': 'https://drive.google.com/uc?id=1pGBa15D9yepQPlrK67NhwlqTCRt4U5q0',
            '400': 'https://drive.google.com/uc?id=1CL99D_ZzX9eOhOj7H2Wco8FVktxkLuZx',
            'all': 'https://drive.google.com/uc?id=1vKDvJWk_OZ8TPqzzD_oMhtSXwCrLUkTv'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"RibLeft11 Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'rib_left_11{subgroup}.zip')

...
class GluteusMediusLeftDataset:
    '''
    This class is for the gluteus medius left segmentation dataset from the total segmentator dataset.
    You can get more information about it using `info()` function.

    ### Example usage

    ```Python
    from pycad.dataset.segmentation.TotalSegmentator import GluteusMediusLeftDataset
    
    gluteus_medius_left_dataset = GluteusMediusLeftDataset()
    gluteus_medius_left_dataset.info()  # Print dataset information
    gluteus_medius_left_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=1pJ-AldQWM2dEwwcpyeqIGf9lHEI7akYc',
            '200': 'https://drive.google.com/uc?id=13LVtstlzC-izLmfS5dogQtAcTPZE6PWK',
            '400': 'https://drive.google.com/uc?id=1XWnTOXrXIsMfUzoHJrxEKdy_9KE0-a0p',
            'all': 'https://drive.google.com/uc?id=1_O-zNcaR4ziPcieitgcInHLlvCd4Ysdn'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Gluteus Medius Left Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'gluteus_medius_left{subgroup}.zip')

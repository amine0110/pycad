...
class GallbladderDataset:
    '''
    This class is for the gallbladder segmentation dataset from the total segmentator dataset.
    ...
    gallbladder_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=1Ze5vlmReDN8hr6NwL3m2jKXDC93b7Hi5',
            '200': 'https://drive.google.com/uc?id=1ahvn66EE0z5LJnKOcA1lMC5sj_kRCkNF',
            '400': 'https://drive.google.com/uc?id=1E_Kf95ZHTIPJyR9umpd7S8h-zi9UGHEm',
            'all': 'https://drive.google.com/uc?id=11NhSQ2QIAuILIiLXP2c7RSxdnIPXyQoK'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Gallbladder Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'gallbladder{subgroup}.zip')

...
class LungLowerLobeLeftDataset:
    '''
    This class is for the lung lower lobe left segmentation dataset from the total segmentator dataset.
    You can get more information about it using `info()` function.

    ### Example usage

    ```Python
    from pycad.dataset.segmentation.TotalSegmentator import LungLowerLobeLeftDataset
    
    lung_lower_lobe_left_dataset = LungLowerLobeLeftDataset()
    lung_lower_lobe_left_dataset.info()  # Print dataset information
    lung_lower_lobe_left_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=1DmDpnsaZzScK7NFLTVzb-C7qsoE2aVwr',
            '200': 'https://drive.google.com/uc?id=1_DHqa62ZJZJPxhWVTP3TsBjQlhw8Ks9E',
            '400': 'https://drive.google.com/uc?id=1g6uqy0_e47TJ54w0szHhQTY0nUkq-hlC',
            'all': 'https://drive.google.com/uc?id=10OqZvXPwyRwvRP4f8OyQKuWOqqN91D2p'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Lung Lower LobeLeft Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'lung_lower_lobe_left{subgroup}.zip')

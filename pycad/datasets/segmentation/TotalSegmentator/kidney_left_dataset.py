...
class KidneyLeftDataset:
    '''
    This class is for the kidney left segmentation dataset from the total segmentator dataset.
    You can get more information about it using `info()` function.

    ### Example usage

    ```Python
    from pycad.dataset.segmentation.decathlon import KidneyLeftDataset
    
    kidney_left_dataset = KidneyLeftDataset()
    kidney_left_dataset.info()  # Print dataset information
    kidney_left_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=1M3quWp2k0pEVVYBLt6UwG12yIRDUT9tM',
            '200': 'https://drive.google.com/uc?id=1zzNEZ7XAL4eJ1uHm6giMILErEcFsPdgR',
            '400': 'https://drive.google.com/uc?id=1QVYryvnAncUOTIQK7B4gdHAoE2O1KbKT',
            'all': 'https://drive.google.com/uc?id=13d-NtszRxQ-y9qrVQ3Bs9dN4-mYGNbrH'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Kidney Left Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'kidney_left{subgroup}.zip')

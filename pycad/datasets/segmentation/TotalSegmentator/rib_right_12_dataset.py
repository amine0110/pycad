...
class RibRight12Dataset:
    '''
    This class is for the rib right 12 segmentation dataset from the total segmentator dataset.
    You can get more information about it using `info()` function.

    ### Example usage

    ```Python
    from pycad.dataset.segmentation.TotalSegmentator import RibRight12Dataset
    
    rib_right_12_dataset = RibRight12Dataset()
    rib_right_12_dataset.info()  # Print dataset information
    rib_right_12_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=1hlLWMpMp3CY9Fh1uzp6HqrleLhZv2rFu',
            '200': 'https://drive.google.com/uc?id=1Bh8v084Bht0L1MvJDKdizpTttHH9BhPe',
            '400': 'https://drive.google.com/uc?id=1JHnq1UIvH9YKQDGA24kJcr0X7V7gKYy-',
            'all': 'https://drive.google.com/uc?id=1fja_QlhdH6l5adfTaHk755FM4-HUc4xM'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Rib Right 12 Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'rib_right_12{subgroup}.zip')

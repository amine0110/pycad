...
class TracheaDataset:
    '''
    This class is for the trachea segmentation dataset from the total segmentator dataset.
    You can get more information about it using `info()` function.

    ### Example usage

    ```Python
    from pycad.dataset.segmentation.TotalSegmentator import TracheaDataset
    
    trachea_dataset = TracheaDataset()
    trachea_dataset.info()  # Print dataset information
    trachea_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=1ikjOOj0he0EkdexCcKch26qqJ-S5MEin',
            '200': 'https://drive.google.com/uc?id=1P_xCP2F2e_sF11zjrjtAYwraKTflb5Hc',
            '400': 'https://drive.google.com/uc?id=1jh_K8XT05DRPnGyf0pJ7nnmOp1DVZJtD',
            'all': 'https://drive.google.com/uc?id=15RzCN_hdAwRmvcaOVySMPjpVgh6_GfAH'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Trachea Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'trachea{subgroup}.zip')

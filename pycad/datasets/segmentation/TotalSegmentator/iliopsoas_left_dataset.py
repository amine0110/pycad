...
class IliopsoasLeftDataset:
    '''
    This class is for the iliopsoas left segmentation dataset from the total segmentator dataset.
    You can get more information about it using `info()` function.

    ### Example usage

    ```Python
    from pycad.dataset.segmentation.decathlon import IliopsoasLeftDataset
    
    iliopsoas_left_dataset = IliopsoasLeftDataset()
    iliopsoas_left_dataset.info()  # Print dataset information
    iliopsoas_left_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=1cOPVjhvlZ9Qe4isHzDG9PDrLY3S9t5Wf',
            '200': 'https://drive.google.com/uc?id=1Rr8BHVHyRateS7WQqrZkl12y_A1Q0fRO',
            '400': 'https://drive.google.com/uc?id=159dpxstQhl0eHRwq5fwM1kdFgA8FfZtr',
            'all': 'https://drive.google.com/uc?id=10VCDCObM-_QBng-nwc2_aEMqyNzU09dV'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Iliopsoas Left Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'iliopsoas_left{subgroup}.zip')

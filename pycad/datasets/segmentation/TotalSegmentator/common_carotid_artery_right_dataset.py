...
class CommonCarotidArteryRightDataset:
    '''
    This class is for the common carotid artery right segmentation dataset from the total segmentator dataset.
    You can get more information about it using `info()` function.

    ### Example usage

    ```Python
    from pycad.dataset.segmentation.TotalSegmentator import CommonCarotidArteryRightDataset
    
    common_carotid_artery_right_dataset = CommonCarotidArteryRightDataset()
    common_carotid_artery_right_dataset.info()  # Print dataset information
    common_carotid_artery_right_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=1jjkd0ClbslfKMUsYW2-VxfwUlDazR5pC',
            '200': 'https://drive.google.com/uc?id=1qcrWv92NLvtVBszC7QzszMjqfREGYqb9',
            '400': 'https://drive.google.com/uc?id=1k8qGu0yjc4h_uKp_kbbKikGQ2PsJEYOR',
            'all': 'https://drive.google.com/uc?id=1vzt8hXc-oZRT5xai1lWqApH4eHewYsWm'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Common Carotid Artery Right Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'common_carotid_artery_right{subgroup}.zip')

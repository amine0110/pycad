...
class VertebraeC4Dataset:
    '''
    This class is for the vertebrae c4 segmentation dataset from the total segmentator dataset.
    You can get more information about it using `info()` function.

    ### Example usage

    ```Python
    from pycad.dataset.segmentation.TotalSegmentator import VertebraeC4Dataset
    
    vertebrae_c4_dataset = VertebraeC4Dataset()
    vertebrae_c4_dataset.info()  # Print dataset information
    vertebrae_c4_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=1U8bFh_GUNXNv11kpD9LPf86KMMv7AHYA',
            '200': 'https://drive.google.com/uc?id=1TvikivumcVWGL7Wrw9oXFQAEa4BB49Aq',
            '400': 'https://drive.google.com/uc?id=1NAizwFbQ3S9X_8ly8-gr_dDi6aoKkyJc',
            'all': 'https://drive.google.com/uc?id=1NUZ7UuDA8FcLC9eNOyILCx1mFjWrtaqi'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Vertebrae C4 Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'vertebrae_c4{subgroup}.zip')

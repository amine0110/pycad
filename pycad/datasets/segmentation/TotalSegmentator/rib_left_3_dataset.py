...
class RibLeft3Dataset:
    '''
    This class is for the rib left 3 segmentation dataset from the total segmentator dataset.
    You can get more information about it using `info()` function.

    ### Example usage

    ```Python
    from pycad.dataset.segmentation.TotalSegmentator import RibLeft3Dataset
    
    rib_left_3_dataset = RibLeft3Dataset()
    rib_left_3_dataset.info()  # Print dataset information
    rib_left_3_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=1bPYwLEEy7ieEXizXASJgRAPXOjNabiId',
            '200': 'https://drive.google.com/uc?id=1Ik1NLX21Uzl6R-6i3OYWX6OtTd_H1kb1',
            '400': 'https://drive.google.com/uc?id=12pHyqoEQjCfwYl5CjNp8PQ5S8d9sY6p-',
            'all': 'https://drive.google.com/uc?id=1ZjUyORooSYcSG4PMZ-T9bcfoXrY0To4s'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Rib Left 3 Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'rib_left_3{subgroup}.zip')

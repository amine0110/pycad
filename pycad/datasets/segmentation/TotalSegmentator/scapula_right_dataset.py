...
class ScapulaRightDataset:
    '''
    This class is for the scapula right segmentation dataset from the total segmentator dataset.
    You can get more information about it using `info()` function.

    ### Example usage

    ```Python
    from pycad.dataset.segmentation.TotalSegmentator import ScapulaRightDataset
    
    scapula_right_dataset = ScapulaRightDataset()
    scapula_right_dataset.info()  # Print dataset information
    scapula_right_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=1lXD7ipxBfUASNbPrROcpy4Okoe_iug5H',
            '200': 'https://drive.google.com/uc?id=1Zb-HCbrMQ6xnlmjm4W1n2djs18-nZh6n',
            '400': 'https://drive.google.com/uc?id=1028lTrE5_zqKzgGBTnFhCteaBsyZ70_9',
            'all': 'https://drive.google.com/uc?id=15Oc9K0b7OXDK5eYHJlmpXJiihCo9w6dL'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Scapula Right Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'scapula_right{subgroup}.zip')

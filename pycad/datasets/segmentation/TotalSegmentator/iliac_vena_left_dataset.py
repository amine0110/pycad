...
class IliacVenaLeftDataset:
    '''
    This class is for the iliac vena left segmentation dataset from the total segmentator dataset.
    You can get more information about it using `info()` function.

    ### Example usage

    ```Python
    from pycad.dataset.segmentation.TotalSegmentator import IliacVenaLeftDataset
    
    iliac_vena_left_dataset = IliacVenaLeftDataset()
    iliac_vena_left_dataset.info()  # Print dataset information
    iliac_vena_left_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=1nD2CLh8yw-2fCelOwKPaEwQ_4T1HkCOX',
            '200': 'https://drive.google.com/uc?id=1mX7hfvd9Zhe6Nb7DJT1yYVDw6_RyGfVC',
            '400': 'https://drive.google.com/uc?id=1WfJey9nHA08XJhbBc7f77GRhWHU0xpBC',
            'all': 'https://drive.google.com/uc?id=1FkQeurLKKojSa3lomJVsOaJHpUW1sDsl'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Iliac Vena Left Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'iliac_vena_left{subgroup}.zip')

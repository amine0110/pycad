...
class ScapulaLeftDataset:
    '''
    This class is for the scapula left segmentation dataset from the total segmentator dataset.
    You can get more information about it using `info()` function.

    ### Example usage

    ```Python
    from pycad.dataset.segmentation.TotalSegmentator import ScapulaLeftDataset
    
    scapula_left_dataset = ScapulaLeftDataset()
    scapula_left_dataset.info()  # Print dataset information
    scapula_left_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=1oGU0jhbS-FhWNosXAO6PQyTrDRKGeol8',
            '200': 'https://drive.google.com/uc?id=1L3yqcCPeqpJjuWGPCa7PpT1i_YzCFM9d',
            '400': 'https://drive.google.com/uc?id=1eC5Zyi54sphvgzO9Ex5P1SoaAnR1GF4z',
            'all': 'https://drive.google.com/uc?id=1mimyB-12a5V9lL6NizOuCygdegWL0eKZ'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Scapula Left Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'scapula_left{subgroup}.zip')

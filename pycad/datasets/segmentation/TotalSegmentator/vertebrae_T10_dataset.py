...
class VertebraeT10Dataset:
    '''
    This class is for the vertebrae t10 segmentation dataset from the total segmentator dataset.
    You can get more information about it using `info()` function.

    ### Example usage

    ```Python
    from pycad.dataset.segmentation.TotalSegmentator import VertebraeT10Dataset
    
    vertebrae_t10_dataset = VertebraeT10Dataset()
    vertebrae_t10_dataset.info()  # Print dataset information
    vertebrae_t10_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=18ZqGIp4cV0Ay9oy2__6RDtuw5PsRCWwW',
            '200': 'https://drive.google.com/uc?id=1lIq0xy9vwUFKDzOXYdURB1ETdTdfF2Nh',
            '400': 'https://drive.google.com/uc?id=16chM2fedro-ScCwHxIyt97gdsvQ6S1IA',
            'all': 'https://drive.google.com/uc?id=1shDrI484iFrSuh_lFsl5b_0GFP3Hb1HA'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Vertebrae T10 Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'vertebrae_t10{subgroup}.zip')

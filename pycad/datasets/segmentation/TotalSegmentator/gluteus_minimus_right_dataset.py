...
class GluteusMinimusRightDataset:
    '''
    This class is for the gluteus minimus right segmentation dataset from the total segmentator dataset.
    You can get more information about it using `info()` function.

    ### Example usage

    ```Python
    from pycad.dataset.segmentation.TotalSegmentator import GluteusMinimusRightDataset
    
    gluteus_minimus_right_dataset = GluteusMinimusRightDataset()
    gluteus_minimus_right_dataset.info()  # Print dataset information
    gluteus_minimus_right_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=1I-eO-eO7q7nkUnINpfeX8AeIIvWRBLit',
            '200': 'https://drive.google.com/uc?id=1BzJflsoV5Zobx0omx6JTmIa_Fm9cWPej',
            '400': 'https://drive.google.com/uc?id=17zFUvc4QrlfcAuXkLovLU9iD5LrYaAuG',
            'all': 'https://drive.google.com/uc?id=1hMHdK-qO9Gk82mSdd8H1bWOglxKU43yz'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Gluteus Minimus Right Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'gluteus_minimus_right{subgroup}.zip')

...
class RibLeft6Dataset:
    '''
    This class is for the rib left 6 segmentation dataset from the total segmentator dataset.
    You can get more information about it using `info()` function.

    ### Example usage

    ```Python
    from pycad.dataset.segmentation.TotalSegmentator import RibLeft6Dataset
    
    rib_left_6_dataset = RibLeft6Dataset()
    rib_left_6_dataset.info()  # Print dataset information
    rib_left_6_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=1iqCz6RgjF8TqVQWHG3knyftwX-I_txyk',
            '200': 'https://drive.google.com/uc?id=19cGm6bshB3x0hlnVCBYWxoJv8ujsj2NO',
            '400': 'https://drive.google.com/uc?id=1npABdrwRrWR41Dpeo1NxJ2ZWigi1aT3Y',
            'all': 'https://drive.google.com/uc?id=1gzBO21lGjgnFpjuTS40pSuUojJtOVJTi'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Rib Left 6 Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'rib_left_6{subgroup}.zip')

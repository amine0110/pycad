...
class VertebraeC6Dataset:
    '''
    This class is for the vertebrae c6 segmentation dataset from the total segmentator dataset.
    You can get more information about it using `info()` function.

    ### Example usage

    ```Python
    from pycad.dataset.segmentation.TotalSegmentator import VertebraeC6Dataset
    
    vertebrae_c6_dataset = VertebraeC6Dataset()
    vertebrae_c6_dataset.info()  # Print dataset information
    vertebrae_c6_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=1MOBZhQYoywiKQpoRuxG6b9eURB5zbw75',
            '200': 'https://drive.google.com/uc?id=1fz6N9GU0bKfUmkzzuXpfC4wQuMGsXVud',
            '400': 'https://drive.google.com/uc?id=1gdFmFPZyPcp79YqCGsNrIsVwsqK1rmOt',
            'all': 'https://drive.google.com/uc?id=1Fg58f0T9r4dW7YYLITisn77VEix7OjvO'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Vertebrae C6 Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'vertebrae_c6{subgroup}.zip')

...
class RibLeft2Dataset:
    '''
    This class is for the rib left 2 segmentation dataset from the total segmentator dataset.
    You can get more information about it using `info()` function.

    ### Example usage

    ```Python
    from pycad.dataset.segmentation.TotalSegmentator import RibLeft2Dataset
    
    rib_left_2_dataset = RibLeft2Dataset()
    rib_left_2_dataset.info()  # Print dataset information
    rib_left_2_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=1LCAfA0GU3JAe0to8C57aDZwVPSPgQzYj',
            '200': 'https://drive.google.com/uc?id=1ug0h_fcn9liSbW38EufMXUCuC-mfImvi',
            '400': 'https://drive.google.com/uc?id=1Mdgz1ZSAhEQorgShbdMdQ8wPXXwOO2JJ',
            'all': 'https://drive.google.com/uc?id=1x_QeoM7Q1ch3ZX8dL934Se8eVJvliKPK'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Rib Left 2 Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'rib_left_2{subgroup}.zip')

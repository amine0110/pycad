...
class SmallBowelDataset:
    '''
    This class is for the small bowel segmentation dataset from the total segmentator dataset.
    You can get more information about it using `info()` function.

    ### Example usage

    ```Python
    from pycad.dataset.segmentation.TotalSegmentator import SmallBowelDataset
    
    small_bowel_dataset = SmallBowelDataset()
    small_bowel_dataset.info()  # Print dataset information
    small_bowel_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=1VRXTZRjRD6MfgF0sjhTKFlpLUF8NHQj1',
            '200': 'https://drive.google.com/uc?id=1i784vNUTmrz_OqRI8zgGJxTH2GN_xEXZ',
            '400': 'https://drive.google.com/uc?id=1psRW3DowVPMevfT7WGCiU9_mY6lGCshp',
            'all': 'https://drive.google.com/uc?id=1oWY11muJBeHyveg49klF1rrTxjZdoII8'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Small Bowel Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'small_bowel{subgroup}.zip')

...
class RibRight7Dataset:
    '''
    This class is for the rib right 7 segmentation dataset from the total segmentator dataset.
    You can get more information about it using `info()` function.

    ### Example usage

    ```Python
    from pycad.dataset.segmentation.TotalSegmentator import RibRight7Dataset
    
    rib_right_7_dataset = RibRight7Dataset()
    rib_right_7_dataset.info()  # Print dataset information
    rib_right_7_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=1m8dLYjOFqHPUgfPFhbxXUOxDfITNE-ev',
            '200': 'https://drive.google.com/uc?id=1W356D7lCigKcSOsoCGfKQKbATibKshq5',
            '400': 'https://drive.google.com/uc?id=1a2aQxDFsCDEkYFlPtfExz3evvVliEi5U',
            'all': 'https://drive.google.com/uc?id=104PMNSFOvbJzERkIBD1QLia23OpZbd9_'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Rib Right 7 Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'rib_right_7{subgroup}.zip')

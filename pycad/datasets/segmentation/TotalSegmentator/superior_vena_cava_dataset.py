...
class SuperiorVenaCavaDataset:
    '''
    This class is for the superior vena cava segmentation dataset from the total segmentator dataset.
    You can get more information about it using `info()` function.

    ### Example usage

    ```Python
    from pycad.dataset.segmentation.TotalSegmentator import SuperiorVenaCavaDataset
    
    superior_vena_cava_dataset = SuperiorVenaCavaDataset()
    superior_vena_cava_dataset.info()  # Print dataset information
    superior_vena_cava_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=1a_h0Wn-8zLheUYwSLRIezyiMGWIcExcG',
            '200': 'https://drive.google.com/uc?id=1o75aJGC_ZKzbNCeDXFXoHX9iNmu33Bse',
            '400': 'https://drive.google.com/uc?id=1EGUAbISdj0iCi6i4mLDTIe1OmCRnHWgV',
            'all': 'https://drive.google.com/uc?id=1OzoOwx2eEfEDPIi1VZkucOQKvAC88QXu'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Superior Vena Cava Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'superior_vena_cava{subgroup}.zip')

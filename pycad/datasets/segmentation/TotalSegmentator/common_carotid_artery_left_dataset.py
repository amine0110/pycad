...
class CommonCarotidArteryLeftDataset:
    '''
    This class is for the common carotid artery left segmentation dataset from the total segmentator dataset.
    ...
    common_carotid_artery_left_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=1urH2MEEDiwJaoYi4VMJ7aYP5sBzJVDac',
            '200': 'https://drive.google.com/uc?id=19JKVF8PhwV8NxdNkfaZ2ypLSy_--Vsh2',
            '400': 'https://drive.google.com/uc?id=1bmFytW2eXny4YTbmpGY8lISbSvM5D9nW',
            'all': 'https://drive.google.com/uc?id=1y4hCAYA4LsSYeFHnSYiat_rGjrBEigOg'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Common Carotid Artery Left Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'common_carotid_artery_left{subgroup}.zip')

...
class StomachDataset:
    '''
    This class is for the stomach segmentation dataset from the total segmentator dataset.
    You can get more information about it using `info()` function.

    ### Example usage

    ```Python
    from pycad.dataset.segmentation.TotalSegmentator import StomachDataset
    
    stomach_dataset = StomachDataset()
    stomach_dataset.info()  # Print dataset information
    stomach_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=15GhGeNZn6C9TMOgtCGNMlQyEXoOS7k_o',
            '200': 'https://drive.google.com/uc?id=1p9ErUrG6FRnIawCmjVSD59MC_Po0TzLf',
            '400': 'https://drive.google.com/uc?id=1V4Lq-8wjegYFVpExxWU0INkrlgNtCgM1',
            'all': 'https://drive.google.com/uc?id=12VUL0JlLnxqm1tCzKazaOuxztd9ucupw'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Stomach Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'stomach{subgroup}.zip')

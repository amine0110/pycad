...
class VertebraeC3Dataset:
    '''
    This class is for the vertebrae c3 segmentation dataset from the total segmentator dataset.
    You can get more information about it using `info()` function.

    ### Example usage

    ```Python
    from pycad.dataset.segmentation.TotalSegmentator import VertebraeC3Dataset
    
    vertebrae_c3_dataset = VertebraeC3Dataset()
    vertebrae_c3_dataset.info()  # Print dataset information
    vertebrae_c3_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=1SXYeQgylOTioxq_VOFhvUHUWocRXdVHS',
            '200': 'https://drive.google.com/uc?id=1y_1oFUZeIyMS8eoUhPlJZkYtxrCHsufk',
            '400': 'https://drive.google.com/uc?id=1Nd3FCIaoT5iqog2m2ytVOIpaJ41IP545',
            'all': 'https://drive.google.com/uc?id=1pW_ecJeSs7OMk5CeQfjAjoSjC2ZB48r7'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Vertebrae C3 Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'vertebrae_c3{subgroup}.zip')

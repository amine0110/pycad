...
class CostalCartilagesDataset:
    '''
    This class is for the costal cartilages segmentation dataset from the total segmentator dataset.
    ...
    costal_cartilages_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=1zsXnCi1xRXbh7zl7L9w1hJj0jNqGWhA2',
            '200': 'https://drive.google.com/uc?id=1sOij2r8_YBnAS3lXwIvTKaiw4ZHJREaM',
            '400': 'https://drive.google.com/uc?id=1EkEYseSnkP875eFG7NybbN__rgc2wLZB',
            'all': 'https://drive.google.com/uc?id=1bz7fqdv7B_E4psURHIntcC5B1S8ayTDs'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Costal Cartilages Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'costal_cartilages{subgroup}.zip')

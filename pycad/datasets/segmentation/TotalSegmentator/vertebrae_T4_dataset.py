...
class VertebraeT4Dataset:
    '''
    This class is for the vertebrae t4 segmentation dataset from the total segmentator dataset.
    You can get more information about it using `info()` function.

    ### Example usage

    ```Python
    from pycad.dataset.segmentation.TotalSegmentator import VertebraeT4Dataset
    
    vertebrae_t4_dataset = VertebraeT4Dataset()
    vertebrae_t4_dataset.info()  # Print dataset information
    vertebrae_t4_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=1O9VEDcNu2iZd2oiyvMwbMtfwb3LhDvmC',
            '200': 'https://drive.google.com/uc?id=1B-8QABff_pPgRb1ruEdNW9l1VQqZP6h1',
            '400': 'https://drive.google.com/uc?id=1RpOwEcLRI_q7aaZevzLTRabroaSqyTr1',
            'all': 'https://drive.google.com/uc?id=1TIsc40MjsvM2XRn3Bg_OI8r0_n-xj84y'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Vertebrae T4 Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'vertebrae_t4{subgroup}.zip')

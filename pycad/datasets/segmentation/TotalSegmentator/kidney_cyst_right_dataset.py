...
class KidneyCystRightDataset:
    '''
    This class is for the kidney cyst right segmentation dataset from the total segmentator dataset.
    You can get more information about it using `info()` function.

    ### Example usage

    ```Python
    from pycad.dataset.segmentation.TotalSegmentator import KidneyCystRightDataset
    
    kidney_cyst_right_dataset = KidneyCystRightDataset()
    kidney_cyst_right_dataset.info()  # Print dataset information
    kidney_cyst_right_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=139ay8dPYkRpChyMXXhXzovudD3AJw6sI',
            '200': 'https://drive.google.com/uc?id=1rG2O39LiY-bQYC7Kq-GcZzL6clevGxSY',
            '400': 'https://drive.google.com/uc?id=1UNFi1eqtwJw2P8jbj7Fy_6FZTRFu1FsX',
            'all': 'https://drive.google.com/uc?id=1AtBm9iTFABoE1ervsngL5g3F4Myth-cP'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Kidney Cyst Right Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'kidney_cyst_right{subgroup}.zip')

...
class HumerusRightDataset:
    '''
    This class is for the humerus right segmentation dataset from the total segmentator dataset.
    ...
    humerus_right_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=16eMAQiHmVkwUsYvxfGnb6nvuVtQHWCoP',
            '200': 'https://drive.google.com/uc?id=1VVIpG5kxe7QYSYuXIZgnRMc5zQM8uX9S',
            '400': 'https://drive.google.com/uc?id=1RLwjK6igPEtSPjSNuE03os25rxc9Obmb',
            'all': 'https://drive.google.com/uc?id=1z6ploe8MQr0bR_o5RCFunHv0FpjnsO-w'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Humerus Right Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'humerus_right{subgroup}.zip')

...
class RibRight3Dataset:
    '''
    This class is for the rib right 3 segmentation dataset from the total segmentator dataset.
    You can get more information about it using `info()` function.

    ### Example usage

    ```Python
    from pycad.dataset.segmentation.TotalSegmentator import RibRight3Dataset
    
    rib_right_3_dataset = RibRight3Dataset()
    rib_right_3_dataset.info()  # Print dataset information
    rib_right_3_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=1qqbgjbvJPxXuXwEQE1xbAat9BHYQnXEP',
            '200': 'https://drive.google.com/uc?id=1X2mvqkyKHueNKWRSLfjpsgY-UleyQ0mA',
            '400': 'https://drive.google.com/uc?id=1KhvG54XjtCkI_iN7aqoUUb7gIW4uTMSE',
            'all': 'https://drive.google.com/uc?id=18PO2IbC3RcWSEsyvehqd_ZXIaBkd3Df9'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Rib Right 3 Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'rib_right_3{subgroup}.zip')

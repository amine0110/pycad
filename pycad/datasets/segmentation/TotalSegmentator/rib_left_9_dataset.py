...
class RibLeft9Dataset:
    '''
    This class is for the rib left 9 segmentation dataset from the total segmentator dataset.
    You can get more information about it using `info()` function.

    ### Example usage

    ```Python
    from pycad.dataset.segmentation.TotalSegmentator import RibLeft9Dataset
    
    rib_left_9_dataset = RibLeft9Dataset()
    rib_left_9_dataset.info()  # Print dataset information
    rib_left_9_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=1oDOulmBQF2dHEHqsjJKwor01xHFm8yPp',
            '200': 'https://drive.google.com/uc?id=1_gNso_kPNT44t8YwM-O3y2eQKKmwyNac',
            '400': 'https://drive.google.com/uc?id=1E7Y3y-DYYTRyq5SF1RZPHA_1w4S1CSHE',
            'all': 'https://drive.google.com/uc?id=17EuTKvDrtS6aZpVms6JMlGL09HN5zrEE'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Rib Left 9 Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'rib_left_9{subgroup}.zip')

...
class SternumDataset:
    '''
    This class is for the sternum segmentation dataset from the total segmentator dataset.
    You can get more information about it using `info()` function.

    ### Example usage

    ```Python
    from pycad.dataset.segmentation.TotalSegmentator import SternumDataset
    
    sternum_dataset = SternumDataset()
    sternum_dataset.info()  # Print dataset information
    sternum_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=13kOMK3OLe9hdLFLZ-2PvUfK2v40URoRD',
            '200': 'https://drive.google.com/uc?id=1RWDhONHOiqTruQy96ADsO2lWQ-QuAJj3',
            '400': 'https://drive.google.com/uc?id=1S_oLp2DhHX7x0J1aYnrGR6Vn4dZAkZCZ',
            'all': 'https://drive.google.com/uc?id=1tusEJmrnW_BEkwBiRCXbnRqEFtT5ScdK'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Sternum Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'sternum{subgroup}.zip')

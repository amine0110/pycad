...
class GluteusMaximusLeftDataset:
    '''
    This class is for the gluteus maximus left segmentation dataset from the total segmentator dataset.
    ...
    gluteus_maximus_left_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=1QCAGUYUqJToRNS2NfLKw4tTpoAi7IMzf',
            '200': 'https://drive.google.com/uc?id=1nlFgvyZ38Rsz7LwvFOxUtdlCu70MInqq',
            '400': 'https://drive.google.com/uc?id=17rCqR_nsiHnZXZhog0nSO9sLU4hM7Pos',
            'all': 'https://drive.google.com/uc?id=1XwZHqDs7GVuSN0QoEtnD2HT0alL2WMma'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Gluteus Maximus Left Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'gluteus_maximus_left{subgroup}.zip')

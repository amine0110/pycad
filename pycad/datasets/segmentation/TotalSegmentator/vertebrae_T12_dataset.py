...
class VertebraeT12Dataset:
    '''
    This class is for the vertebrae t12 segmentation dataset from the total segmentator dataset.
    You can get more information about it using `info()` function.

    ### Example usage

    ```Python
    from pycad.dataset.segmentation.TotalSegmentator import VertebraeT12Dataset
    
    vertebrae_t12_dataset = VertebraeT12Dataset()
    vertebrae_t12_dataset.info()  # Print dataset information
    vertebrae_t12_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=16EcRssXBfgamvAqTIh5LTmUk55kBEcd8',
            '200': 'https://drive.google.com/uc?id=1qBVScMshT6-kj4fS-wZ7V7YcrVcbSTuD',
            '400': 'https://drive.google.com/uc?id=1sn4RQq-T0DRX_9XdFPT-2IuHySOWxAUi',
            'all': 'https://drive.google.com/uc?id=1LiEhGMDE57DQigkXxtqh1XAyL4M4WaUw'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Vertebrae T12 Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'vertebrae_t12{subgroup}.zip')

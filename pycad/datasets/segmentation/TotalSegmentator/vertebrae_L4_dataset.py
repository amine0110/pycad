...
class VertebraeL4Dataset:
    '''
    This class is for the vertebrae l4 segmentation dataset from the total segmentator dataset.
    You can get more information about it using `info()` function.

    ### Example usage

    ```Python
    from pycad.dataset.segmentation.TotalSegmentator import VertebraeL4Dataset
    
    vertebrae_l4_dataset = VertebraeL4Dataset()
    vertebrae_l4_dataset.info()  # Print dataset information
    vertebrae_l4_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=17G3gu4JU689_wX79-jJLAchVf4d7UJrI',
            '200': 'https://drive.google.com/uc?id=1zx5bcxorQfIsyDNeP45RIoq7tJROIKiz',
            '400': 'https://drive.google.com/uc?id=1D5-GbiE1LCAPU6brclsVFtbDJoT5ACcK',
            'all': 'https://drive.google.com/uc?id=1ZZF4CiT0GyuTEpka1gn3wGc8CL_U22Q3'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Vertebrae L4 Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'vertebrae_l4{subgroup}.zip')

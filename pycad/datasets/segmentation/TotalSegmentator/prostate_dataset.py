...
class ProstateDataset:
    '''
    This class is for the prostate segmentation dataset from the total segmentator dataset.
    You can get more information about it using `info()` function.

    ### Example usage

    ```Python
    from pycad.dataset.segmentation.TotalSegmentator import ProstateDataset
    
    prostate_dataset = ProstateDataset()
    prostate_dataset.info()  # Print dataset information
    prostate_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=1sGH-O60yxn7s_CYPn_RDijfi4yurqzR1',
            '200': 'https://drive.google.com/uc?id=1h7qd21wZqHxVevpT8SNcCwCs4XG6wqHC',
            '400': 'https://drive.google.com/uc?id=1lt9DXqHPqdX-yu0RWEI4KkyQJsnhVmXY',
            'all': 'https://drive.google.com/uc?id=1gsAPUTHTv4UX0pZP9IeftpbN0tVy64dm'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Prostate Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'prostate{subgroup}.zip')

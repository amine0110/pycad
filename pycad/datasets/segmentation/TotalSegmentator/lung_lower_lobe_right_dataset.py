...
class LungLowerLobeRightDataset:
    '''
    This class is for the lung lower lobe right segmentation dataset from the total segmentator dataset.
    You can get more information about it using `info()` function.

    ### Example usage

    ```Python
    from pycad.dataset.segmentation.TotalSegmentator import LungLowerLobeRightDataset
    
    lung_lower_lobe_right_dataset = LungLowerLobeRightDataset()
    lung_lower_lobe_right_dataset.info()  # Print dataset information
    lung_lower_lobe_right_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=1A0VlcDXjw1W5Zh3Tt9bAgBxWCudiVOEI',
            '200': 'https://drive.google.com/uc?id=1iEn-O70L4KrB0z2JeLbzAXoKEKyaIBnQ',
            '400': 'https://drive.google.com/uc?id=1mONbwhuQE3AEX1VRZ2-FY9anEJkqaty7',
            'all': 'https://drive.google.com/uc?id=1yMzwuyn2x3MLYw7bsThJPBRPPc4ljpJv'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Lung Lower LobeRight Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'lung_lower_lobe_right{subgroup}.zip')

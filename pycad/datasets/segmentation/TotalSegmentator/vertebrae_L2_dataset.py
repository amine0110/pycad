...
class VertebraeL2Dataset:
    '''
    This class is for the vertebrae_l2 segmentation dataset from the total segmentator dataset.
    You can get more information about it using `info()` function.

    ### Example usage

    ```Python
    from pycad.dataset.segmentation.TotalSegmentator import VertebraeL2Dataset
    
    vertebrae_l2_dataset = VertebraeL2Dataset()
    vertebrae_l2_dataset.info()  # Print dataset information
    vertebrae_l2_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=1uyoJYxD2lL2S84GNctDvMAKHRDsu3Or6',
            '200': 'https://drive.google.com/uc?id=1W2I9qfcw6Ql1CNlc-ccQmDdEkXvDyV55',
            '400': 'https://drive.google.com/uc?id=1YU1QpRNJTY6otq1YjbtZgpbja7JB0ewG',
            'all': 'https://drive.google.com/uc?id=1XnzpjPCKIHNspnJj7Gze0C86xO-ntSBO'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"VertebraeL2 Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'vertebrae_l2{subgroup}.zip')

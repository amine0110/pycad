...
class PulmonaryVeinDataset:
    '''
    This class is for the pulmonary vein segmentation dataset from the total segmentator dataset.
    You can get more information about it using `info()` function.

    ### Example usage

    ```Python
    from pycad.dataset.segmentation.TotalSegmentator import PulmonaryVeinDataset
    
    pulmonary_vein_dataset = PulmonaryVeinDataset()
    pulmonary_vein_dataset.info()  # Print dataset information
    pulmonary_vein_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=1rD09qMcQTQQ7UqnQDc4fBO-4H4xb1ful',
            '200': 'https://drive.google.com/uc?id=1G1PTn-iOvIwH6FAUQKF5DxmmKZ6pVzca',
            '400': 'https://drive.google.com/uc?id=1FfMYA4UxQEgJtCeTH8_HTKAKUjtBEjp2',
            'all': 'https://drive.google.com/uc?id=1jOXmj5inimc_fQy4xrdEy-QaXxkMJfdB'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"PulmonaryVein Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'pulmonary_vein{subgroup}.zip')

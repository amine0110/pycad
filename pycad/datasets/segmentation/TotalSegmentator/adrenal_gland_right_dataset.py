...
class AdrenalGlandRightDataset:
    '''
    This class is for the adrenal gland Right segmentation dataset from the total segmentator dataset.
    You can get more information about it using `info()` function.

    ### Example usage

    ```Python
    from pycad.dataset.segmentation.TotalSegmentator import AdrenalGlandRightDataset
    
    adrenal_gland_right_dataset = AdrenalGlandRightDataset()
    adrenal_gland_right_dataset.info()  # Print dataset information
    adrenal_gland_right_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=1wTujE2cVg5ZChEzrYEpbAzwNBjk0Xo6f',
            '200': 'https://drive.google.com/uc?id=1364lPxDuSlxc5ZQ81IQD7SH_McvULtR2',
            '400': 'https://drive.google.com/uc?id=1gToRKTfKUjiJ3u0NE3eL9LJsfeCU6fYO',
            'all': 'https://drive.google.com/uc?id=1LUiiaQXBXDnV-pTkAxStGpUzhMWWaNRl'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Adrenal Gland Right Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'adrenal_gland_right{subgroup}.zip')

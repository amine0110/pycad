...
class SubclavianArteryRightDataset:
    '''
    This class is for the subclavian artery right segmentation dataset from the total segmentator dataset.
    You can get more information about it using `info()` function.

    ### Example usage

    ```Python
    from pycad.dataset.segmentation.TotalSegmentator import SubclavianArteryRightDataset
    
    subclavian_artery_right_dataset = SubclavianArteryRightDataset()
    subclavian_artery_right_dataset.info()  # Print dataset information
    subclavian_artery_right_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=1bYwLv68DLtk17G82K7By199eCRY83fbb',
            '200': 'https://drive.google.com/uc?id=13ZYGM2EWkv8_eca3KhBR-duMDUkIrjYj',
            '400': 'https://drive.google.com/uc?id=1vORwi3Dn1h8X9txEIwgYwLUqRqIJhPNo',
            'all': 'https://drive.google.com/uc?id=1QayOQudHRqoVgJMtXH9WZLJ-ou36Ve_a'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Subclavian Artery Right Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'subclavian_artery_right{subgroup}.zip')

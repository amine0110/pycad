...
class AtrialAppendageLeftDataset:
    '''
    This class is for the atrial appendage left segmentation dataset from the total segmentator dataset.
    ...
    atrial_appendage_left_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=1fpRnAxfkqsHhYuCmf_1T74aOiDdp9bNG',
            '200': 'https://drive.google.com/uc?id=16ZkiasNrIakENxL87ek8VdkCwjm1-z-K',
            '400': 'https://drive.google.com/uc?id=1GpG0YUhIAZ-L_lPAkImbTDLYY1LqIBxa',
            'all': 'https://drive.google.com/uc?id=1gKHuuvZzQOhYDuGj25I0kTHLdQSCZ5p-'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Atrial Appendage Left Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'atrial_appendage_left{subgroup}.zip')

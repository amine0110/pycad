...
class PortalVeinSplenicVeinDataset:
    '''
    This class is for the portal vein splenic vein segmentation dataset from the total segmentator dataset.
    You can get more information about it using `info()` function.

    ### Example usage

    ```Python
    from pycad.dataset.segmentation.TotalSegmentator import PortalVeinSplenicVeinDataset
    
    portal_vein_splenic_vein_dataset = PortalVeinSplenicVeinDataset()
    portal_vein_splenic_vein_dataset.info()  # Print dataset information
    portal_vein_splenic_vein_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=11L8uUNryyWM0HXxQGBp10xAmRhbg0ia9',
            '200': 'https://drive.google.com/uc?id=1uGhJWt6veLEot7Z6nNO3_RHDuc9bQKIf',
            '400': 'https://drive.google.com/uc?id=1uliknLTYtRLbwOECE5JzBcSgjUvrwBAd',
            'all': 'https://drive.google.com/uc?id=16KKyK_5WUofB3nvKHlyV5Zq3W6DqTI2g'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Portal Vein Splenic Vein Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'portal_vein_splenic_vein{subgroup}.zip')

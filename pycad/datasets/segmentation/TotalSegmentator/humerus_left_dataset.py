...
class HumerusLeftDataset:
    '''
    This class is for the humerus left segmentation dataset from the total segmentator dataset.
    You can get more information about it using `info()` function.

    ### Example usage

    ```Python
    from pycad.dataset.segmentation.TotalSegmentator import HumerusLeftDataset
    
    humerus_left_dataset = HumerusLeftDataset()
    humerus_left_dataset.info()  # Print dataset information
    humerus_left_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=1F_469od6luLNzSB2xaNvnxosI1UTAAog',
            '200': 'https://drive.google.com/uc?id=1Jq6wpt6OWHNiZPp3iIwjfxT4aIBw_q8O',
            '400': 'https://drive.google.com/uc?id=1QcCvabIRPAXAjhbtqQD93MpfFUKoQITf',
            'all': 'https://drive.google.com/uc?id=1XyGMT3PbNHnh2ynVviqCGkXPfi-lDvX5'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Humerus Left Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'humerus_left{subgroup}.zip')

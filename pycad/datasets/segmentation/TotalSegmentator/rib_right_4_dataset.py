...
class RibRight4Dataset:
    '''
    This class is for the rib right 4 segmentation dataset from the total segmentator dataset.
    You can get more information about it using `info()` function.

    ### Example usage

    ```Python
    from pycad.dataset.segmentation.TotalSegmentator import RibRight4Dataset
    
    rib_right_4_dataset = RibRight4Dataset()
    rib_right_4_dataset.info()  # Print dataset information
    rib_right_4_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=1H-CYRB-7VrRXizYlWVfIBK6Er1ryoLz7',
            '200': 'https://drive.google.com/uc?id=1Ezmter2wmNzTACxDZ11z_IF3JcXyRusm',
            '400': 'https://drive.google.com/uc?id=1jVHxoXe35q7vTz-qILg6UTeXgIZssf2K',
            'all': 'https://drive.google.com/uc?id=19hplxVEXkJUqvQNpEjNQ7B4J6D2tvih9'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Rib Right 4 Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'rib_right_4{subgroup}.zip')

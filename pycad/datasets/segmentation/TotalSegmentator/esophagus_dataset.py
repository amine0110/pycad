...
class EsophagusDataset:
    '''
    This class is for the esophagus segmentation dataset from the total segmentator dataset.
    ...
    esophagus_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=19xzc2A-xMDlKu9xcRpBL3ZYMi5uS_X8K',
            '200': 'https://drive.google.com/uc?id=1WGzcXOQ8XARCNqrnFy8ivxgguzjIlu0Q',
            '400': 'https://drive.google.com/uc?id=1Bd3PnzPmUuawPB7Yo2W4LWMHsEQjKDPr',
            'all': 'https://drive.google.com/uc?id=1D0FxVA07Vz2_FNBSMry6w26gQatqGnid'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Esophagus Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'esophagus{subgroup}.zip')

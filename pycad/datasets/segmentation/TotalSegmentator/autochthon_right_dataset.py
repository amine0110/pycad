...
class AutochthonRightDataset:
    '''
    This class is for the autochthon right segmentation dataset from the total segmentator dataset.
    ...
    autochthon_right_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=1AWwBrJmrwAOFk4GrYjJjF2FDqxOFmz2G',
            '200': 'https://drive.google.com/uc?id=1Nw_PSme0XNDQqcAZnr6SuDWidnXierGD',
            '400': 'https://drive.google.com/uc?id=1RdVPFd8HrXwqqikRzayVwrehhGEptLPg',
            'all': 'https://drive.google.com/uc?id=1m989fZFlqRxyw0Yr-TQ9ze5RnwlpAbLd'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Autochthon Right Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'autochthon_right{subgroup}.zip')

...
class SpinalCordDataset:
    '''
    This class is for the spinal cord segmentation dataset from the total segmentator dataset.
    You can get more information about it using `info()` function.

    ### Example usage

    ```Python
    from pycad.dataset.segmentation.TotalSegmentator import SpinalCordDataset
    
    spinal_cord_dataset = SpinalCordDataset()
    spinal_cord_dataset.info()  # Print dataset information
    spinal_cord_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=1X5OhRl_PPzWCLQ9ZTxGnE3iqaxQ4g7YV',
            '200': 'https://drive.google.com/uc?id=19PD97Mmxs5cQiBlqqygDgxUZjo0fqauo',
            '400': 'https://drive.google.com/uc?id=1UX25oY5cL02-IW9G7fNqDP98D06tgvIg',
            'all': 'https://drive.google.com/uc?id=1gsFgUBEeA3lv8bA9F7WUIQzESfLqqA0K'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Spinal Cord Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'spinal_cord{subgroup}.zip')

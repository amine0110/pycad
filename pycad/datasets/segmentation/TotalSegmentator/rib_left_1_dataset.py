...
class RibLeft1Dataset:
    '''
    This class is for the rib left 1 segmentation dataset from the total segmentator dataset.
    You can get more information about it using `info()` function.

    ### Example usage

    ```Python
    from pycad.dataset.segmentation.TotalSegmentator import RibLeft1Dataset
    
    rib_left_1_dataset = RibLeft1Dataset()
    rib_left_1_dataset.info()  # Print dataset information
    rib_left_1_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=1vbEGkzB1jAGPoBWFBinBGihVetsNNTLa',
            '200': 'https://drive.google.com/uc?id=104n-qTFdF95xFO4lR3jIhaMuhxplD74V',
            '400': 'https://drive.google.com/uc?id=1DAjjrYwAZxDsfHF-zcP9oUNsdzFEyeT8',
            'all': 'https://drive.google.com/uc?id=1r9165BxUEYLeevRcUFFzaY4PJ83CCxt2'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Rib Left 1 Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'rib_left_1{subgroup}.zip')

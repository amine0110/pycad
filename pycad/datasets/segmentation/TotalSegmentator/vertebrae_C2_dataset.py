...
class VertebraeC2Dataset:
    '''
    This class is for the vertebrae c2 segmentation dataset from the total segmentator dataset.
    You can get more information about it using `info()` function.

    ### Example usage

    ```Python
    from pycad.dataset.segmentation.TotalSegmentator import VertebraeC2Dataset
    
    vertebrae_c2_dataset = VertebraeC2Dataset()
    vertebrae_c2_dataset.info()  # Print dataset information
    vertebrae_c2_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=11PPO8iPpJVoaKXt_mTPDaHu6-k-XrPI9',
            '200': 'https://drive.google.com/uc?id=1T_58Dd4fhn0k62XADYeJxPGoy6kYeGZ7',
            '400': 'https://drive.google.com/uc?id=1fD0-GAG7HWo7Kp4FB8PZCgOv-yjBWiXr',
            'all': 'https://drive.google.com/uc?id=1RjZQswcb9qg7qRWRXtRm8ASUBbvw5ELj'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Vertebrae C2 Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'vertebrae_c2{subgroup}.zip')

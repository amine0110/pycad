...
class BrachiocephalicVeinLeftDataset:
    '''
    This class is for the brachiocephalic vein left segmentation dataset from the total segmentator dataset.
    ...
    brachiocephalic vein_left_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=1IiavNwNRuilHusFcZ_WQFWgZRvxz3edY',
            '200': 'https://drive.google.com/uc?id=1Q3G13BWlkhXGD6nZ46ZbFV7AfMqDSja5',
            '400': 'https://drive.google.com/uc?id=1FoIAeCKSwEN4CHKef6M6jKgAIlA0VqyY',
            'all': 'https://drive.google.com/uc?id=1fEkcBoJGfiQhuHBW3gZBCg5dOMQJul1J'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Brachiocephalic Vein Left Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'brachiocephalic_vein_left{subgroup}.zip')

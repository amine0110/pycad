...
class IliacVenaRightDatasetDataset:
    '''
    This class is for the iliac vena right dataset segmentation dataset from the total segmentator dataset.
    You can get more information about it using `info()` function.

    ### Example usage

    ```Python
    from pycad.dataset.segmentation.TotalSegmentator import IliacVenaRightDatasetDataset
    
    iliac_vena_right_dataset_dataset = IliacVenaRightDatasetDataset()
    iliac_vena_right_dataset_dataset.info()  # Print dataset information
    iliac_vena_right_dataset_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=1atcLPAnkkOg0UjrYLn3e-d0LGzvlCmto',
            '200': 'https://drive.google.com/uc?id=1L1n4Wul4tcbyysS9DAMlEp0sVl227-rW',
            '400': 'https://drive.google.com/uc?id=1j6zXqoj83Ov8a-TjYbAvlCrfBIJoo_8f',
            'all': 'https://drive.google.com/uc?id=1hqBwVXDiq1WZTpCqmoCDztIBC5874azo'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Iliac Vena Right Dataset Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'iliac_vena_right_dataset{subgroup}.zip')

...
class InferiorVenaCavaDataset:
    '''
    This class is for the inferior vena cava segmentation dataset from the total segmentator dataset.
    ...
    inferior_vena_cava_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=1iqXvVhlWs6IyBQyVD__1osnfniBzzkl0',
            '200': 'https://drive.google.com/uc?id=1_4ZD_t7Eea8E-S_X13ko_NusRjRSYp29',
            '400': 'https://drive.google.com/uc?id=14Oh5ZGT33tJZoiiT1Gc6NpP1cEa01BEU',
            'all': 'https://drive.google.com/uc?id=1aLwIx2FVx6yMZssBrXbcCrFT1fJIZ2Tv'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Inferior Vena Cava Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'inferior_vena_cava{subgroup}.zip')

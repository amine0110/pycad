...
class VertebraeT6Dataset:
    '''
    This class is for the vertebrae t6 segmentation dataset from the total segmentator dataset.
    You can get more information about it using `info()` function.

    ### Example usage

    ```Python
    from pycad.dataset.segmentation.TotalSegmentator import VertebraeT6Dataset
    
    vertebrae_t6_dataset = VertebraeT6Dataset()
    vertebrae_t6_dataset.info()  # Print dataset information
    vertebrae_t6_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=12DSBA4dTcLms48klvNOIjrUcmROMQTnG',
            '200': 'https://drive.google.com/uc?id=1j4zZjWBui1bUTuS1bM-fzHYsK_DyJCmd',
            '400': 'https://drive.google.com/uc?id=1OCMJJEJMnIYRwkFju-CL3OLjVgnkcwyQ',
            'all': 'https://drive.google.com/uc?id=120eiErqOfPXeqFGWEhxHuH7YLWgYz6Uf'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Vertebrae T6 Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'vertebrae_t6{subgroup}.zip')

...
class RibRight10Dataset:
    '''
    This class is for the rib right 10 segmentation dataset from the total segmentator dataset.
    You can get more information about it using `info()` function.

    ### Example usage

    ```Python
    from pycad.dataset.segmentation.TotalSegmentator import RibRight10Dataset
    
    rib_right_10_dataset = RibRight10Dataset()
    rib_right_10_dataset.info()  # Print dataset information
    rib_right_10_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=1ySilCNm6eLcp8GFsb_shC_uKvQDvANe4',
            '200': 'https://drive.google.com/uc?id=1EkVjUL-EZSPksxlvdR_p0u6juE9ES1KC',
            '400': 'https://drive.google.com/uc?id=1lGtf_t4LuPVSUb3O4DpNwhK7Qj8zbaBk',
            'all': 'https://drive.google.com/uc?id=1ohoMU-d6dJSJ3m2GpM4EvgDmJdbviNtC'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Rib Right 10 Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'rib_right_10{subgroup}.zip')

...
class VertebraeC5Dataset:
    '''
    This class is for the vertebrae c5 segmentation dataset from the total segmentator dataset.
    You can get more information about it using `info()` function.

    ### Example usage

    ```Python
    from pycad.dataset.segmentation.TotalSegmentator import VertebraeC5Dataset
    
    vertebrae_c5_dataset = VertebraeC5Dataset()
    vertebrae_c5_dataset.info()  # Print dataset information
    vertebrae_c5_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=1QAB0B3VO5lvEFcUaax4FLfAViUK1yuNc',
            '200': 'https://drive.google.com/uc?id=1AKJc06pH_Y7PPE32oFbAaha-kZZBMbpk',
            '400': 'https://drive.google.com/uc?id=1LtShP4Btgl3t4kCZv4ccpN2rJXy5Td4B',
            'all': 'https://drive.google.com/uc?id=1G6zKKEXFJjjkFv9INixJ_kJK0lpsBNte'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Vertebrae C5 Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'vertebrae_c5{subgroup}.zip')

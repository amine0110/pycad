...
class RibLeft5Dataset:
    '''
    This class is for the rib left 5 segmentation dataset from the total segmentator dataset.
    You can get more information about it using `info()` function.

    ### Example usage

    ```Python
    from pycad.dataset.segmentation.TotalSegmentator import RibLeft5Dataset
    
    rib_left_5_dataset = RibLeft5Dataset()
    rib_left_5_dataset.info()  # Print dataset information
    rib_left_5_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=1Wp7yS6HTdqJv4W7k-ZKvBGFvXubVjVNb',
            '200': 'https://drive.google.com/uc?id=1nOeYKxSi1ZPZgGUXwvpmXztytahIsb2I',
            '400': 'https://drive.google.com/uc?id=1ekIxODM3sydCoMUUpwWFYVEzEE-F_UWu',
            'all': 'https://drive.google.com/uc?id=1UOcrw1JNvZtM66W2QCiRZLYDWLWwPlW8'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Rib Left 5 Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'rib_left_5{subgroup}.zip')

...
class IliopsoasRightDataset:
    '''
    This class is for the iliopsoas right segmentation dataset from the total segmentator dataset.
    ...
    iliopsoas_right_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=1Ozsg9J9zrKs0ej8yBdnn_0s9FoWvA139',
            '200': 'https://drive.google.com/uc?id=1nqJZvOohYr2Ne7PcJRV1NrhRrKxFbIPB',
            '400': 'https://drive.google.com/uc?id=1khfhovYXnnmWBBh6SmVqVsTdcoQpDuuP',
            'all': 'https://drive.google.com/uc?id=1TDFw05wNTlRTAbeDL3_0WoqnHD5rANFy'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Iliopsoas Right Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'iliopsoas_right{subgroup}.zip')

...
class FemurLeftDataset:
    '''
    This class is for the femur left segmentation dataset from the total segmentator dataset.
    ...
    femur_left_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=12DtqgLNdsOidjBptOann_4cRGcsEPhKo',
            '200': 'https://drive.google.com/uc?id=1fuJ5fMjkBlTmEAU_sgSSTV1TEfxhkNNq',
            '400': 'https://drive.google.com/uc?id=1SPffSWX23WtGH7pOP144b6GatFO7hyUE',
            'all': 'https://drive.google.com/uc?id=1psAU5PdWsqLlTnpkS82zixH_ru8Bw32v'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Femur Left Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'femur_left{subgroup}.zip')

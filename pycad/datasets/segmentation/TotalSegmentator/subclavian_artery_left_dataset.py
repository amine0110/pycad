...
class SubclavianArteryLeftDataset:
    '''
    This class is for the subclavian artery left segmentation dataset from the total segmentator dataset.
    You can get more information about it using `info()` function.

    ### Example usage

    ```Python
    from pycad.dataset.segmentation.TotalSegmentator import SubclavianArteryLeftDataset
    
    subclavian_artery_left_dataset = SubclavianArteryLeftDataset()
    subclavian_artery_left_dataset.info()  # Print dataset information
    subclavian_artery_left_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=1tniAA5OcVQ0Xj7mbzHGOBqtdRW6O0oIU',
            '200': 'https://drive.google.com/uc?id=1SkEIuZMxNrpzSW4rBMPb3bmlnO0eKtin',
            '400': 'https://drive.google.com/uc?id=1XhmIzHGV3oltA_lh8lNUsR0Wvf-XFfCu',
            'all': 'https://drive.google.com/uc?id=1VB9sP8uxDJ-4q9JDTC2YhAsdfHNq6b1j'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Subclavian Artery Left Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'subclavian_artery_left{subgroup}.zip')

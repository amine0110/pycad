...
class SkullDataset:
    '''
    This class is for the skull segmentation dataset from the total segmentator dataset.
    You can get more information about it using `info()` function.

    ### Example usage

    ```Python
    from pycad.dataset.segmentation.TotalSegmentator import SkullDataset
    
    skull_dataset = SkullDataset()
    skull_dataset.info()  # Print dataset information
    skull_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=13dvqTbf5Zoxu_1TvBedZivY4ReY5JrGF',
            '200': 'https://drive.google.com/uc?id=1Mw3_fyEfmrb8eEgp5BeV_U4FdP9VnnAt',
            '400': 'https://drive.google.com/uc?id=1YK5bjrse3ipOXZEbTNfCjCymd1NnJMoy',
            'all': 'https://drive.google.com/uc?id=1aAkdfZC4OQR4rz-4wV1BnuYrfZyZ5C5Z'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Skull Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'skull{subgroup}.zip')

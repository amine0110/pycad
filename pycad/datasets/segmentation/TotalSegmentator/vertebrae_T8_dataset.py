...
class VertebraeT8Dataset:
    '''
    This class is for the vertebrae t8 segmentation dataset from the total segmentator dataset.
    You can get more information about it using `info()` function.

    ### Example usage

    ```Python
    from pycad.dataset.segmentation.TotalSegmentator import VertebraeT8Dataset
    
    vertebrae_t8_dataset = VertebraeT8Dataset()
    vertebrae_t8_dataset.info()  # Print dataset information
    vertebrae_t8_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=11sS3LsMfTp2EmqcY7XgaAT30JCPTiDf2',
            '200': 'https://drive.google.com/uc?id=1KEli85KROOTiAGsIl0g5z1kg6uwF4iwC',
            '400': 'https://drive.google.com/uc?id=1z37krWrpMRdmhkDKfMUsxKVwRgB4R3Pl',
            'all': 'https://drive.google.com/uc?id=1logXk_1zUzJ_I88v9QdVIi2PS4NiZ7nc'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Vertebrae T8 Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'vertebrae_t8{subgroup}.zip')

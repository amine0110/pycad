...
class BrainDataset:
    '''
    This class is for the brain segmentation dataset from the total segmentator dataset.
    ...
    brain_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=1KdLXvI0NPpOd2NirUMbPnhmruqnpW12X',
            '200': 'https://drive.google.com/uc?id=15ddEeILUYeblTX7zBAKfPgnzh2JzKhsx',
            '400': 'https://drive.google.com/uc?id=1m2Ty29DqdgTyxbLkzzhcWhwLvgLH1L4o',
            'all': 'https://drive.google.com/uc?id=1UNq0rAS-y699UkyMGWVZsDuaUVIOBXtY'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Brain Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'brain{subgroup}.zip')

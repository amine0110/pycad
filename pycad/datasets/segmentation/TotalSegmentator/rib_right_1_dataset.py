...
class RibRight1Dataset:
    '''
    This class is for the rib right 1 segmentation dataset from the total segmentator dataset.
    You can get more information about it using `info()` function.

    ### Example usage

    ```Python
    from pycad.dataset.segmentation.TotalSegmentator import RibRight1Dataset
    
    rib_right_1_dataset = RibRight1Dataset()
    rib_right_1_dataset.info()  # Print dataset information
    rib_right_1_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=1guSlpWugWfxjb4jFp1klDnHgRxE9RPg4',
            '200': 'https://drive.google.com/uc?id=1qdN_RM7Pv_wwPrjE-J2VHZqvnb_cc9ku',
            '400': 'https://drive.google.com/uc?id=1CTUQTg-y8CtXAukiRl9dzQ0D07_SPUXW',
            'all': 'https://drive.google.com/uc?id=12mUQJ9fUPIFRaE7QMric3JNeJ5PyEpu7'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Rib Right 1 Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'rib_right_1{subgroup}.zip')

...
class AortaDataset:
    '''
    This class is for the aorta segmentation dataset from the total segmentator dataset.
    ...
    aorta_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    ...
    def info(self):
        print(f"Aorta Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'aorta{subgroup}.zip')

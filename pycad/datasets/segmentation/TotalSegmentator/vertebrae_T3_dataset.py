...
class VertebraeT3Dataset:
    '''
    This class is for the vertebrae t3 segmentation dataset from the total segmentator dataset.
    You can get more information about it using `info()` function.

    ### Example usage

    ```Python
    from pycad.dataset.segmentation.TotalSegmentator import VertebraeT3Dataset
    
    vertebrae_t3_dataset = VertebraeT3Dataset()
    vertebrae_t3_dataset.info()  # Print dataset information
    vertebrae_t3_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=1rXO_PdKcNj3o-buTQvBXDTnFtRe-rJNl',
            '200': 'https://drive.google.com/uc?id=1sssKksrU-r7Rx6UV6uuVC7lwuVOxQY7n',
            '400': 'https://drive.google.com/uc?id=1K78aHWM9qA-wcfGRLlU6aFXOOHw6CRVd',
            'all': 'https://drive.google.com/uc?id=1phKAwZZcda2k3IUZKnKJeeP2FaTzZtWB'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Vertebrae T3 Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'vertebrae_t3{subgroup}.zip')

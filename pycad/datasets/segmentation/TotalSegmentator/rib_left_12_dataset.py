...
class RibLeft12Dataset:
    '''
    This class is for the rib left 12 segmentation dataset from the total segmentator dataset.
    You can get more information about it using `info()` function.

    ### Example usage

    ```Python
    from pycad.dataset.segmentation.TotalSegmentator import RibLeft12Dataset
    
    rib_left_12_dataset = RibLeft12Dataset()
    rib_left_12_dataset.info()  # Print dataset information
    rib_left_12_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=1qjonKxGK7599J91XZldLipeFRGH5sa9T',
            '200': 'https://drive.google.com/uc?id=1bC13uDYFGJuHye3x3bRchZeIb-Yoj27S',
            '400': 'https://drive.google.com/uc?id=11UoThobk8IxHMkX2_nEQXMf5kkj3eJPa',
            'all': 'https://drive.google.com/uc?id=10wVTAZd87Rktwf_C-Zk2q6U-W7D2DvwO'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"RibLeft12 Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'rib_left_12{subgroup}.zip')

...
class VertebraeL1Dataset:
    '''
    This class is for the vertebrae l1 segmentation dataset from the total segmentator dataset.
    You can get more information about it using `info()` function.

    ### Example usage

    ```Python
    from pycad.dataset.segmentation.TotalSegmentator import VertebraeL1Dataset
    
    vertebrae_l1_dataset = VertebraeL1Dataset()
    vertebrae_l1_dataset.info()  # Print dataset information
    vertebrae_l1_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=1no8ZDPp4JUVJNuE7c5e3uK1ZQqbygWIw',
            '200': 'https://drive.google.com/uc?id=1fhRSlW6HR4zz0r4H3wGmI32RW4351BsI',
            '400': 'https://drive.google.com/uc?id=12m-TApLvG8OeL2kU_36NJub7hGRZLFff',
            'all': 'https://drive.google.com/uc?id=130lToQQekayr9Of92bwww44CWCu8-YW_'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Vertebrae L1 Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'vertebrae_l1{subgroup}.zip')

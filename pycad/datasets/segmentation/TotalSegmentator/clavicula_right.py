...
class ClaviculaRightDataset:
    '''
    This class is for the clavicula right segmentation dataset from the total segmentator dataset.
    You can get more information about it using `info()` function.

    ### Example usage

    ```Python
    from pycad.dataset.segmentation.TotalSegmentator import ClaviculaRightDataset
    
    clavicula_right_dataset = ClaviculaRightDataset()
    clavicula_right_dataset.info()  # Print dataset information
    clavicula_right_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    ...
    def info(self):
        print(f"Clavicula Right Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'clavicula_right{subgroup}.zip')

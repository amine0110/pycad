...
class GluteusMediusRightDataset:
    '''
    This class is for the gluteus medius right segmentation dataset from the total segmentator dataset.
    ...
    gluteus_medius_right_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=13AIPzzUvAN6bXmr1ahxWBxpEH5Ay_YYp',
            '200': 'https://drive.google.com/uc?id=1ikLgX6yZosH6zgZpfMZ6EDU90PO3AFh2',
            '400': 'https://drive.google.com/uc?id=1aPLHXhOhJ-TrE3izyDvoqeUfFMfumroO',
            'all': 'https://drive.google.com/uc?id=1HT5uCCOPznLuHXUXGrYNJxdG4AnO3dy8'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Gluteus Medius Right Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'gluteus_medius_right{subgroup}.zip')

...
class LungUpperLobeLeftDataset:
    '''
    This class is for the lung upper lobe left segmentation dataset from the total segmentator dataset.
    You can get more information about it using `info()` function.

    ### Example usage

    ```Python
    from pycad.dataset.segmentation.TotalSegmentator import LungUpperLobeLeftDataset
    
    lung_upper_lobe_left_dataset = LungUpperLobeLeftDataset()
    lung_upper_lobe_left_dataset.info()  # Print dataset information
    lung_upper_lobe_left_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=1T5M7KkWOsw1la5Ln--0DUda_OMFDQWHu',
            '200': 'https://drive.google.com/uc?id=1RFSX46ftkXXKrz7qf4ImnUS4dAogcPaG',
            '400': 'https://drive.google.com/uc?id=1g9uCDc4d1mM1aZsJ7xgTTEqm-f8M1NtF',
            'all': 'https://drive.google.com/uc?id=1b0ugrlRUOVSnzaXgEuTqJc2mujiPJq4_'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Lung Upper Lobe Left Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'lung_upper_lobe_left{subgroup}.zip')

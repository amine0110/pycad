...
class RibLeft4Dataset:
    '''
    This class is for the rib left 4 segmentation dataset from the total segmentator dataset.
    You can get more information about it using `info()` function.

    ### Example usage

    ```Python
    from pycad.dataset.segmentation.TotalSegmentator import RibLeft4Dataset
    
    rib_left_4_dataset = RibLeft4Dataset()
    rib_left_4_dataset.info()  # Print dataset information
    rib_left_4_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=12ZG5DvDrLv0fMJQIwK3MyCMpWTYK3iWT',
            '200': 'https://drive.google.com/uc?id=1dlPJTi53l9-t8JpJaYAPpgD527ag8Thy',
            '400': 'https://drive.google.com/uc?id=1YPvVn-Ms3CFAKB-JZDAAIlm4CtR_-ErC',
            'all': 'https://drive.google.com/uc?id=1lIzaCQLp8nn50sOo8aRDRwWUjY93a1qC'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Rib Left 4 Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'rib_left_4{subgroup}.zip')

...
class RibLeft10Dataset:
    '''
    This class is for the rib left 10 segmentation dataset from the total segmentator dataset.
    You can get more information about it using `info()` function.

    ### Example usage

    ```Python
    from pycad.dataset.segmentation.TotalSegmentator import RibLeft10Dataset
    
    rib_left_10_dataset = RibLeft10Dataset()
    rib_left_10_dataset.info()  # Print dataset information
    rib_left_10_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=1RmYfUpjGf1brVgVAMJuWTWNxxgYiWeWt',
            '200': 'https://drive.google.com/uc?id=1YEqr4_tc9T0kkKS4MvmWRYru7tvAe7_G',
            '400': 'https://drive.google.com/uc?id=1HOCPlfQ_07M1zjmkfljLKT4dMQPgvVZ7',
            'all': 'https://drive.google.com/uc?id=1Z6t-FmPG0-zhm0mAUbE32DXfRGKCil34'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Rib Left 10 Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'rib_left_10{subgroup}.zip')

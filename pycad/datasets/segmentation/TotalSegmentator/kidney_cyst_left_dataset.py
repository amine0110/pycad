...
class KidneyCystLeftDataset:
    '''
    This class is for the kidney cyst left segmentation dataset from the total segmentator dataset.
    You can get more information about it using `info()` function.

    ### Example usage

    ```Python
    from pycad.dataset.segmentation.TotalSegmentator import KidneyCystLeftDataset
    
    kidney_cyst_left_dataset = KidneyCystLeftDataset()
    kidney_cyst_left_dataset.info()  # Print dataset information
    kidney_cyst_left_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=1MX2yLSziiA0FZFrce9DlK_lzX4hjwyst',
            '200': 'https://drive.google.com/uc?id=1UMryBbidYdDxLB5Xb5Eg3praF_rlghzt',
            '400': 'https://drive.google.com/uc?id=1-8QYIZJr1-7pkQfB87aK2J0xO7ZX5Dve',
            'all': 'https://drive.google.com/uc?id=1Y-1tPYsXOcJ7UOCOJPYjN0GNF4UNsPSF'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Kidney Cyst Left Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'kidney_cyst_left{subgroup}.zip')

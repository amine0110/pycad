...
class BrachiocephalicVeinRightDataset:
    '''
    This class is for the brachiocephalic vein right segmentation dataset from the total segmentator dataset.
    You can get more information about it using `info()` function.

    ### Example usage

    ```Python
    from pycad.dataset.segmentation.TotalSegmentator import BrachiocephalicVeinRightDataset
    
    brachiocephalic vein_right_dataset = BrachiocephalicVeinRightDataset()
    brachiocephalic vein_right_dataset.info()  # Print dataset information
    brachiocephalic vein_right_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=1HV-rSON3HxRHwpmcp-F49_BfQE_y4pxS',
            '200': 'https://drive.google.com/uc?id=1OIK051IxuvYuOGG-YsT40FKfH6MGwpDQ',
            '400': 'https://drive.google.com/uc?id=1sDP8JFvKVb8AI3qDo8kr1YiOR6VE6L6k',
            'all': 'https://drive.google.com/uc?id=1e9_LncZRoRaFmg_ph7Jmd_J_6vUVY8qW'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Brachiocephalic Vein Right Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'brachiocephalic_vein_right{subgroup}.zip')

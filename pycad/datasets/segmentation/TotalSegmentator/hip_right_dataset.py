...
class HipRightDataset:
    '''
    This class is for the hip right segmentation dataset from the total segmentator dataset.
    ...
    hip_right_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=1J7EbOGKGqoSzK73-oUWy731f7eI9r_YF',
            '200': 'https://drive.google.com/uc?id=1bxI_BTizIevPU3zm9zvl7LEjK0NM_19J',
            '400': 'https://drive.google.com/uc?id=1JBm_h6jKUvi0XAnyXusenZriQdG8lHLZ',
            'all': 'https://drive.google.com/uc?id=1rQSGKsy95lWupsxHGKLrZEnETvZiFa3b'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Hip Right Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'hip_right{subgroup}.zip')

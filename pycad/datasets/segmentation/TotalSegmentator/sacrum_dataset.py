...
class SacrumDataset:
    '''
    This class is for the sacrum segmentation dataset from the total segmentator dataset.
    You can get more information about it using `info()` function.

    ### Example usage

    ```Python
    from pycad.dataset.segmentation.TotalSegmentator import SacrumDataset
    
    sacrum_dataset = SacrumDataset()
    sacrum_dataset.info()  # Print dataset information
    sacrum_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=1DKphiRIgS6IfC3_oyYVznN_wzBdj2CDH',
            '200': 'https://drive.google.com/uc?id=1-lTBbokqyPvFsHOoP_8hPNvCH-TY6GTx',
            '400': 'https://drive.google.com/uc?id=1MKvxC9PZj27V2KW4Vsw7q2OAtpoz-JhQ',
            'all': 'https://drive.google.com/uc?id=1fhbh5PuFD8_rPNpomPSM4RGIr_44p3YV'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Sacrum Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'sacrum{subgroup}.zip')

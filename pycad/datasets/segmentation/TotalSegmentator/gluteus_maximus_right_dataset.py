...
class GluteusMaximusRightDataset:
    '''
    This class is for the gluteus maximus right segmentation dataset from the total segmentator dataset.
    You can get more information about it using `info()` function.

    ### Example usage

    ```Python
    from pycad.dataset.segmentation.TotalSegmentator import GluteusMaximusRightDataset
    
    gluteus_maximus_right_dataset = GluteusMaximusRightDataset()
    gluteus_maximus_right_dataset.info()  # Print dataset information
    gluteus_maximus_right_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=1x9fq1S52mkPG5xphaRcios_2h4O_4lIU',
            '200': 'https://drive.google.com/uc?id=1kOcPVDEyFrks__1SMpwwaqscuiBCkY_I',
            '400': 'https://drive.google.com/uc?id=18JR2T6xt9p5YxCUPqZnIpgv3u3E19RHG',
            'all': 'https://drive.google.com/uc?id=1GPrSiJmabEA3tG5XG1JuMKVz3EgMeFCg'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Gluteus Maximus Right Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'gluteus_maximus_right{subgroup}.zip')

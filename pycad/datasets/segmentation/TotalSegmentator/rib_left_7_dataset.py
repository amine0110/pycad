...
class RibLeft7Dataset:
    '''
    This class is for the rib left 7 segmentation dataset from the total segmentator dataset.
    You can get more information about it using `info()` function.

    ### Example usage

    ```Python
    from pycad.dataset.segmentation.TotalSegmentator import RibLeft7Dataset
    
    rib_left_7_dataset = RibLeft7Dataset()
    rib_left_7_dataset.info()  # Print dataset information
    rib_left_7_dataset.download('100')  # Download and extract subgroup 100
    ```
    '''
    def __init__(self, dataset_size=1225):
        self.dataset_size = dataset_size
        self.dataset_subgroups = {
            '100': 'https://drive.google.com/uc?id=1tMKRwxEI2tpbyZqFsHh-mUW8tsqtbC71',
            '200': 'https://drive.google.com/uc?id=1rZqIxPtcFyLd_d2d1DrvWxkfxEFd8lfB',
            '400': 'https://drive.google.com/uc?id=1taBr9zS18Q2HH9iQfLpunJByK-lcgGyZ',
            'all': 'https://drive.google.com/uc?id=1B0oICid1QZw1HYETWALCNOtnFDjcwnEx'
        }
        self.base_path = 'datasets/'

    def info(self):
        print(f"Rib Left 7 Dataset from Total Segmentator dataset. This is a collection of CT scans with means these are 3D volumes.")
    ...
    def _download_and_extract(self, url, path, subgroup):
        ...
        try:
            file_path = os.path.join(path, f'rib_left_7{subgroup}.zip')
